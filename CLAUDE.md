...
| 想改什麼 | 動哪 |
|---------|------|
| 癌別／副作用選項 | script 開頭 `CANCER_TYPES`、`SE_ITEMS`、`SE_GRADES` 陣列 |
| JSON 匯入邏輯與欄位對映 | `importFromObject(data)`；格式文件 `JSON-FORMAT.md`（**改欄位名要三處同步：此函式、格式文件、sample.json**） |
| 清單外值的下拉處理 | `setSelect(sel, val)`：動態補 option、不失真 |
| 日期遮罩（20260101 → 2026/01/01） | `fmtDate(v)` ＋ document 層 input 委派（`.date-input`） |
...
8. 匯入 sample.json → 3 位病人帶入、期間顯示 2026/07/06 ～ 2026/07/20；再匯入一次 → 出現清空／附加詢問
9. 交班期間輸入 20260101 → 即時顯示 2026/01/01
10. 部署驗證：推 main → Pages 開啟 → 頁面原始碼無任何病人資料（sample.json 僅虛構測試資料）
```
...
- **V0.4.0（2026-07）** 副作用拆成項目＋分級（Gr.1–4）兩個下拉；JSON 批次匯入（`importFromObject`，含清空／附加詢問、清單外值動態補 option、錯誤防禦），附 `JSON-FORMAT.md` 與 `sample.json`；交班期間改西元日期輸入遮罩（`fmtDate`）。鎖定版轉換同步涵蓋 date-input 與新下拉。
...
2. 反向匯出 JSON（現況→檔案，配合匯入形成可再編輯的資料迴圈，取代已 deprecated 的 P1 序列化路線）
3. 病人排序切換（依剩餘次數升冪）
